...
class pornjshimmer(object):
    ...
    def __init__(self, ledCount):
        ...
        self.directions = list(1 for i in range(0, self.ledCount))
        self.colors = list(1 for i in range(0, self.ledCount))

        for i in range(0, self.ledCount):
            self.values[i] = 0
            #self.maxValues[i] = random.randrange(0,self.ledMax)
            self.maxValues[i] = random.randint(self.ledMax/3.0,self.ledMax)
            self.deaths[i] = self.maxValues[i] + self.stepSize / 2 + (.5 * random.randrange(0,self.ledMax))
            self.directions[i] = 1
            self.colors[i] = random.randrange(0,2)
    ...
    def getData(self):
        ...
        for i in range(0, self.ledCount):
            ...
            if self.colors[i] == 0:
            #pink
                data += makePixel(self.values[i]*(1), self.values[i]*(0), self.values[i]*(3.0/10))
            else:
            #orange
                data += makePixel(self.values[i]*(1), self.values[i]*(1.0/5), self.values[i]*(0))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for pixel in range(0, 14*3):
            ...
            bt.sendPixel(ord(data[pixel*3+2]), ord(data[pixel*3+1]), ord(data[pixel*3+0]))
        bt.show()
